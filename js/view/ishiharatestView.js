 ...
 * Camada View: Manipulação visual das duas grelhas numéricas independentes e resultados.
 */
    ...
    renderFinalResults(quizData, userAnswers, metrics) {
        ...
        if (tbody) {
            ...
            quizData.forEach((plate, index) => {
                ...
                // Validação de acerto estrito (baseado no valor ideal de visão normal)
                const isCorrect = userAns.toString().trim() === plate.correct.toString().trim();
                
                // Define uma string customizada caso a placa seja de classificação avançada
                let correctDisplay = plate.correct;
                if(plate.protan) {
                    correctDisplay = `${plate.correct} (Protan: ${plate.protan} / Deutan: ${plate.deutan})`;
                }
                ...
                row.innerHTML = `
                    ...
                    <td style="font-size: 13px;">${correctDisplay}</td>
                `;
        ...
        if (scoreEl) {
            scoreEl.textContent = `Triagem Geral: ${metrics.percentage.toFixed(0)}% de acertos (${metrics.hits}/${metrics.total})`;
        }

        if (diagLabel) {
            diagLabel.textContent = metrics.diagnosis;
            diagLabel.style.color = metrics.color;
        }
    }
        ...
        if (historyData.length === 0) {
            ...
            historyData.forEach(item => {
                ...
                tr.innerHTML = `
                    ...
                    <td style="font-weight:600;">${item.diagnosis}</td>
                `;
